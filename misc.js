...
function keyboardNew(key) {Modal.confirm("New Project", "Do you really want to create a new project?<br><br>If data is already loaded it will be erased!", () => {editor.destroy(); editor.loadClean(); return true;}, null, "Create", "Abort", "small", true);}
...
function doExport() {
	...
	let content = editor.print();
	if(content == null) {
		ToastModal.open("Nothing to export");
		return true;
	}
	
	downloadFile = new File([content], filename+".damage", {type: 'text/plain'});
	let downloadURI = URL.createObjectURL(downloadFile);
	let link = document.createElement('a');
	link.href = downloadURI;
	link.download = filename+".damage";
...
function openHelp() {
	Modal.open(
		...
		"A web-based editor to easily create and edit Starbound's .damage files.<br>"+
		"Import an existing .damage file or start from scratch by adding things through the tabs in the center.<br><br>"+
		"<b>Hotkeys:</b><br>"+
...
function startup() {
	...
	document.getElementById('newProjectButton').addEventListener('click', (e) => {
		Modal.confirm("New Project", "Do you really want to create a new project?<br><br>If data is already loaded it will be erased!", () => {editor.destroy(); editor.loadClean(); return true;}, null, "Create", "Abort", "small", true);
	});
	...
	document.getElementById('compressionLevel').addEventListener('change', function(e) {
		editor.compressionLevel = this.value;
	});
}